...
from .llm_appraisal import appraise_from_text
from .appraisal_log import maybe_log as log_appraisal_row
from .cognition import interpret, lens, read_lens
...
def _recall(text, name, personality=None):
    ...
    try:
        ...
        from .memory import recurrence, recall     # heavy: numpy
...
def _recall_beliefs(embedding, personality, name):
    ...
    if embedding is None:
        return []
    try:
        from .belief_memory import recall_beliefs   # heavy: numpy
...
def _store_memory(text, embedding, appraisal, push, personality, name):
    ...
    *interpreted* appraisal -- what the character actually perceived -- so future recalls
    ...
    if embedding is None:
        return
    try:
        from .memory import create_memory          # heavy: numpy
...
def _form_beliefs(personality, char_name):
    ...
    try:
        ...
        from .belief_memory import load_belief_embeddings, append_belief_embeddings
